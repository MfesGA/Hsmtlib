...
\haddockbeginargs
\haddockdecltt{::} & \haddockdecltt{Solvers} & Avaliable\haddockid{Solvers}.
 \\
                                               \haddockdecltt{->} & \haddockdecltt{Mode} & Avaliable \haddocktt{Modes}, Online, Script, Context.
 \\
                                                                                           \haddockdecltt{->} & \haddockdecltt{String} & The desired SMT Logic.
 \\
                                                                                                                                         \haddockdecltt{->} & \haddockdecltt{Maybe SolverConfig} & A customized Configuration for the Solver.
 \\
                                                                                                                                                                                                   \haddockdecltt{->} & \haddockdecltt{Maybe FilePath} & A possible alternate path to save the Script.
 \\
                                                                                                                                                                                                                                                         \haddockdecltt{->} & \haddockdecltt{IO Solver} & \\
\end{tabulary}\par
...
\par
There exists three \haddockid{Mode}s that a solver can be used, Online,
Context and Script.
\par
...
solver.
\par
In script \haddockid{Mode} a file is created in a desired file path, if Nothing is passed
...
\par
In context \haddockid{Mode} two lists are kept, a list with all the commands given and
a list that has the results of commands that demand some output from the solver.
\par
The functions in this mode behave has in script \haddockid{Mode} being the diference that
...
\par
The operators and there uses can be found in \haddockid{Solver}
\par
\begin{itemize}
\item
